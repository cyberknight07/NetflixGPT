...
function GPTSearchBar() {
    ...
    const handleGptSearchClick=async()=>{
        ...
        const gptQuery = "Act as a Movie Recommendation System and suggest some movies for the Query: "+searchText.current.value +" only give the name of most 5 relevant Movies,comma seperated like the example result given ahead. Example Result: Gadar,sholay,don,dangal,spiderman.";
        try {
            ...
            const responseText = gptResults.candidates?.[0]?.content?.parts?.[0]?.text;
            ...
            console.error("Error initializing GeminiAI model:", error);
            return;
        }
    }
